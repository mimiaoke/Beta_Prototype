...
// 	private $acceptedTypes = ["image/jpeg", "image/png", "image/gif", "image/tiff", "image/svg+xml"];
	...
	public function store() {
		...
		$pic->position = $this->filedata["position"];
		$pic->lat = $this->filedata["lat"];
		$pic->lng = $this->filedata["lng"];
		$pic->save();
	}
	...
	// 	var_dump($this->filedata);
		...
		$picname = $f3->get('POST.picDescription');
 		$picPosition = $f3->get('POST.position');
		$picLat = $f3->get('POST.lat');
		$picLng = $f3->get('POST.lng');
		$this->filedata["title"] = $picname;		// add the title to filedata for later use
		$this->filedata["position"] = $picPosition;
		$this->filedata["lat"] = $picLat;
		$this->filedata["lng"] = $picLng;
		$this->store();
	...
	public function infoService($picID=0) {
		...
		if ($picID == 0) {
			foreach ($list as $record) {
				...
				$recordData["lat"] = $record["lat"];
				$recordData["lng"] = $record["lng"];
		...
		$recordData["position"] = $pic["position"];
		$recordData["lat"] = $pic["lat"];
		$recordData["lng"] = $pic["lng"];
		return $recordData;
	}

	public function getImageByPosition($position=0) {
		global $f3;
		$returnData = array();
		$pic=new DB\SQL\Mapper($f3->get('DB'),$this->pictable);	// create DB query mapper object
		$list = $pic->find();
