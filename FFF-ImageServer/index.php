...
$f3->route('GET /',
    function ($f3) {
        $f3->set('html_title','Server Home');
        $f3->set('content','content/index.html');
        echo Template::instance()->render('layout/index.html');
    }
);
...
$f3->route('GET /upload',
    function($f3) {
        ...
        $f3->set('content','view_upload.html');
...
$f3->route('POST /upload',
    function($f3) {
        ...
        if ($filedata = $is->upload()) {						// if this is null, upload failed
            $f3->set('filedata', $filedata);
            $f3->set('html_title','Image Server Home');
            $f3->set('content','content/view_upload.html');
            echo template::instance()->render('layout/view_upload.html');
        }
    }
);
...
$f3->route('GET|POST /getImageByPosition',
    function($f3) {
        $is = new ImageServer;
        $info = $is->getImageByPosition(0);
        echo json_encode($info);
    }
);

$f3->route('GET|POST /getImageByPosition/@position',
    function($f3) {
        $is = new ImageServer;
        $info = $is->getImageByPosition($f3->get('PARAMS.position'));
        echo json_encode($info);
    }
);
...
// simple route for AJAX page


// simple route for View Uploaded Images

$f3->route('GET /view_upload',
    function($f3) {
        $is = new ImageServer;
        $info = $is->infoService(0);
        $f3->set('datalist', $info);
...
$f3->route('GET /canvas',
    function ($f3) {
        $f3->set('html_title','Server Home');
        $f3->set('content','content/canvas.html');
        echo Template::instance()->render('layout/canvas.html');
    }
);


$f3->route('GET /selectcanvas',
    function ($f3) {
        $f3->set('html_title','Server Home');
        $f3->set('content','content/selectcanvas.html');
        echo Template::instance()->render('layout/selectcanvas.html');
    }
);
